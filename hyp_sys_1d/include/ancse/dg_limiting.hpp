...
template <class Limiter> class DGLimiting : public Limiting {
public:
  DGLimiting(const Grid &grid, const DGHandler &dg_handler,
             const Limiter &limiter)
      : n_cells(grid.n_cells), n_ghost(grid.n_ghost), dx(grid.dx),
        dg_handler(dg_handler), limiter(limiter) {}
  ...
  operator()(const Eigen::VectorXd &uc0, const Eigen::VectorXd &um0,
             const Eigen::VectorXd &up0, const Eigen::VectorXd &um,
