 ...
 */
template <class NumericalFlux> class DGRateOfChange : public RateOfChange {
public:
  DGRateOfChange(const Grid &grid, const std::shared_ptr<Model> &model,
                 const NumericalFlux &numerical_flux,
                 const PolynomialBasis &poly_basis, const DGHandler &dg_handler)
      : grid(grid), model(model), numerical_flux(numerical_flux),
        poly_basis(poly_basis), dg_handler(dg_handler) {
