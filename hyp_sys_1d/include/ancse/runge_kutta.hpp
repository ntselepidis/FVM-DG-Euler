...
public:
  ...
  virtual void operator()(Eigen::MatrixXd &u1, const Eigen::MatrixXd &u0,
                          double dt) const = 0;
};
...
public:
  ForwardEuler(std::shared_ptr<RateOfChange> rate_of_change_,
               std::shared_ptr<BoundaryCondition> boundary_condition_,
               std::shared_ptr<Limiting> limiting_, int n_rows, int n_cols)
      : super(std::move(rate_of_change_), std::move(boundary_condition_),
              std::move(limiting_)),
        dudt(n_rows, n_cols) {}
...
public:
  ...
  virtual void operator()(Eigen::MatrixXd &u1, const Eigen::MatrixXd &u0,
    ...
    (*rate_of_change)(dudt, u0); // dudt = L(u0)
    u1 = u0 + dt * dudt;         // u1 = u0 + dt*L(u0)
    post_euler_step(u1);         // BC [+ Limiting] on u1

    (*rate_of_change)(dudt, u1); // dudt = L(u1)
    u1 = u1 + dt * dudt;         // u1 = u1 + dt*L(u1)

    u1 = 0.5 * (u0 + u1); // RK averaging
    post_euler_step(u1);  // BC [+ Limiting] on u1
  }
...
make_runge_kutta(const nlohmann::json &config,
                 const std::shared_ptr<RateOfChange> &rate_of_change,
                 const std::shared_ptr<BoundaryCondition> &boundary_condition,
                 const std::shared_ptr<Limiting> &dg_limiting, int n_rows,
                 int n_cols);
