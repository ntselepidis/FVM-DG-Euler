...
Eigen::MatrixXd ic(const F &f, const Grid &grid,
  ...
  Eigen::MatrixXd u0 = Eigen::MatrixXd::Zero(n_vars * n_coeff, grid.n_cells);
...
TimeLoop make_dg(const nlohmann::json &config, const Grid &grid,
                 const PolynomialBasis &poly_basis, const DGHandler &dg_handler,
  ...
  auto dg_rate_of_change = make_dg_rate_of_change(
      config, grid, model, poly_basis, dg_handler, simulation_time);
  auto dg_limiting = make_dg_limiting(config, grid, dg_handler);
  auto time_integrator =
      make_runge_kutta(config, dg_rate_of_change, boundary_condition,
                       dg_limiting, n_vars * n_coeff, n_cells);
  auto cfl_condition = make_cfl_condition(grid, model, dg_handler, cfl_number);
  auto snapshot_writer = std::make_shared<JSONSnapshotWriter<DG>>(
      grid, model, dg_handler, simulation_time,
      std::string(config["output_dir"]), std::string(config["output_file"]));

  return TimeLoop(simulation_time, time_integrator, cfl_condition,
                  snapshot_writer);
}
