...
template <class F> Eigen::MatrixXd ic(const F &f, const Grid &grid) {
...
TimeLoop make_fvm(const nlohmann::json &config, const Grid &grid,
  ...
  auto fvm_rate_of_change =
      make_fvm_rate_of_change(config, grid, model, simulation_time);
  auto boundary_condition =
      make_boundary_condition(n_ghost, config["boundary_condition"]);
  auto time_integrator = make_runge_kutta(config, fvm_rate_of_change,
                                          boundary_condition, n_vars, n_cells);
  auto cfl_condition = make_cfl_condition(grid, model, cfl_number);
  auto snapshot_writer = std::make_shared<JSONSnapshotWriter<FVM>>(
      grid, model, simulation_time, std::string(config["output_dir"]),
