...
void DGRateOfChange<NumericalFlux>::eval_numerical_flux(
    Eigen::MatrixXd &dudt, const Eigen::MatrixXd &u0) const {
  ...
  for (int j = n_ghost - 1; j < n_cells - n_ghost; j++) {
    ...
    for (int i = 0; i < n_vars; i++) {
      dudt.col(j).segment(i * n_coeff, n_coeff) += fL(i) * phiR - fR(i) * phiL;
    }
  }
}
...
template <class NumericalFlux>
void DGRateOfChange<NumericalFlux>::eval_volume_integral(
    Eigen::MatrixXd &dudt, const Eigen::MatrixXd &u0) const {
  ...
  for (int j = n_ghost; j < n_cells - n_ghost; j++) {
    ...
    for (int k = 0; k < n_quad_points; k++) {
      ...
      Eigen::VectorXd u_k = dg_handler.build_sol(u0.col(j), quad_points(k));
...
#define REGISTER_NUMERICAL_FLUX(token, FluxType, flux)                         \
  if (config["flux"] == (token)) {                                             \
    return std::make_shared<DGRateOfChange<FluxType>>(grid, model, flux,       \
                                                      poly_basis, dg_handler); \
  }

std::shared_ptr<RateOfChange>
make_dg_rate_of_change(const nlohmann::json &config, const Grid &grid,
