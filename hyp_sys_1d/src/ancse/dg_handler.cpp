...
Eigen::VectorXd DGHandler ::build_sol(const Eigen::VectorXd &u,
...
/// build cell average
Eigen::MatrixXd DGHandler ::build_cell_avg(const Eigen::MatrixXd &u) const {
  auto n_cells = u.cols();
...
void DGHandler ::compute_limit_coeffs(Eigen::MatrixXd &u, Eigen::MatrixXd &um,
  ...
  if (n_coeff == 3) {
    ...
    for (int j = 0; j < n_cells; j++) {
      for (int i = 0; i < n_vars; i++) {
        ...
        u.col(j).segment(i * n_coeff + 1, n_coeff - 1) = invA * b;
      }
    }
  } else { /* if ( n_coeff == 2 ) */
    for (int j = 0; j < n_cells; j++) {
