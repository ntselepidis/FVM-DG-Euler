...
JSONSnapshotWriter<FVM>::JSONSnapshotWriter(
    const Grid &grid_, std::shared_ptr<Model> &model_,
    std::shared_ptr<SimulationTime> simulation_time_, std::string basedir_,
    std::string basename_)
    : grid(grid_), model(model_), simulation_time(std::move(simulation_time_)),
      basedir(std::move(basedir_)), basename(std::move(basename_)) {
  fs::create_directories(basedir);
}

void JSONSnapshotWriter<FVM>::operator()(const Eigen::MatrixXd &u) const {
  std::string filename =
      basedir + "/" + basename + fmt::format("{:04d}.json", k_output);
  k_output += 1;
...
JSONSnapshotWriter<DG>::JSONSnapshotWriter(
    const Grid &grid_, std::shared_ptr<Model> &model_,
    const DGHandler &dg_handler_,
    std::shared_ptr<SimulationTime> simulation_time_, std::string basedir_,
    std::string basename_)
    : grid(grid_), model(model_), dg_handler(dg_handler_),
      simulation_time(std::move(simulation_time_)),
      basedir(std::move(basedir_)), basename(std::move(basename_)) {
  fs::create_directories(basedir);
}

void JSONSnapshotWriter<DG>::operator()(const Eigen::MatrixXd &u) const {
  std::string filename =
      basedir + "/" + basename + fmt::format("{:04d}.json", k_output);
  k_output += 1;
