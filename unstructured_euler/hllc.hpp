...
EulerState hllc(const EulerState &uL, const EulerState &uR) {
    ...
    /* speeds */
    const double sL = euler::minEigenValue( uL, uR ); // TODO: Implement Einfeldt-Batten sL
    const double sR = euler::maxEigenValue( uL, uR ); // TODO: Implement Einfeldt-Batten sR
    const double s_star =
    ...
    uL_star(2) = zL;
    uL_star(3) = ( EL / rhoL ) +
        ( s_star - vL )*( s_star + ( pL / (rhoL*(sL-vL)) ) );
    ...
    uR_star(2) = zR;
    uR_star(3) = ( ER / rhoR ) +
        ( s_star - vR )*( s_star + ( pR / (rhoR*(sR-vR)) ) );
